...
const INGR_ID_2 = '643d69a5c3f7b9001cfa093e';
...
describe('Создание заказа', function () {
  beforeEach(() => {
    ...
    window.localStorage.setItem('refreshToken', 'somerefreshtoke');
  });
  ...
  it('Отправка заказа', function () {
    ...
    cy.get('[data-cy="' + INGR_ID + '"]')
      .children('button')
      .click();
    cy.get('[data-cy="' + INGR_ID_2 + '"]')
      .children('button')
      .click();
    cy.get('[data-cy="order"]').click();

    cy.get('[id^="modal"]').find('h2').contains('67372');
    cy.get('[id^="modal"]').find('button').eq(1).click();
    cy.get('[data-cy="total-price"]').children('p').should('have.text', '0');
  });
});
